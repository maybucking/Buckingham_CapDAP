...
# Overview
Here are the three hypothesis that I would like to explore for my data analysis capstone project
...
###Now that the data is cleaned up lets explore the data.

Lets look at the relationship between success rate for kestrels and years that the box is up.

```{r}
ggplot(kestrels_long, aes(Years.Up, X.Successful)) + 
  geom_point()
```
Based on this graph it appears that boxes that are up for more years have a higher success rate


Now lets look at the relationship between years the box is up and the number of chicks banded
```{r}
kestrels_long$Years.Up <- as.factor(kestrels_long$Years.Up)
```
```{r}
ggplot(kestrels_long, aes(Years.Up, Num_Chicks_Banded)) + 
  geom_col()
```
Based on this graph I would assume that the longer the box is up, the more chicks are banded
...
summary(lmNewData)
```
The results of the summary show that there is a p-value of 0.0001304 meaning that there is a significant correlation between Years Up and Success rate. The summary also shows that the estimated y - intercept is 38.305 % and would increase by 1.369 % each additional year. This supports that the years up and percent success rate has a positive correlation.
...
Generate a scatterplot to look at the initial relationship between the variables
```{r}
...
Making number of chicks binary (present or not present) using if_else.
```{r}
...
To plot this we will create a new data set called plot_chiks
```{r}
